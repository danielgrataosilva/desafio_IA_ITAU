...
4. **Estabilização e último reteste real**: `parallel_tool_calls=False` eliminou o erro anterior de validação causado pelo nome de tool contaminado. Em uma tentativa intermediária, `response_format={"type":"json_object"}` exigiu menção explícita a JSON; isso foi corrigido no prompt final. Na última execução de `CLI-014`, as quatro chamadas à API foram concluídas tecnicamente e o modelo escolheu e executou corretamente `historico_cliente` e `perfil_canal`. Contudo, o JSON final não trouxe `nivel_risco`, `tipologia_suspeita`, `red_flags` nem `justificativa`; o Pydantic o rejeitou corretamente. Não houve parecer válido, validação factual final ou lote Groq. A execução consumiu aproximadamente 7.312 tokens em 4,321 s, sem retries.
...
O Groq apresentou latência inferior aos testes Gemini e passou os testes mínimo e sintético de tool calling. Ainda assim, a extrapolação factual semântica, o erro `400` anterior de validação de tool e o JSON final incompatível com o schema no último reteste indicaram incompatibilidades específicas desse provider/modelo no fluxo real, com risco de regressão dentro do prazo do desafio.

A decisão definitiva de engenharia foi **não promover Groq ao caminho principal** e preservar Gemini como caminho final estável. Isso não significa que o Groq seja inviável em geral; significa apenas que não atingiu o critério de estabilidade definido para esta entrega.
